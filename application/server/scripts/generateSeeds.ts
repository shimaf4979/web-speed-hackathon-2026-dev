...
function loadExistingSounds(): SoundSeed[] {
  const currentSeeds = readFileSync(path.resolve(seedsDir, "sounds.jsonl"), "utf8");
  return currentSeeds
    .split("\n")
    .map((line) => line.trim())
    .filter((line) => line !== "")
...
const EXISTING_SOUNDS = loadExistingSounds();
...
function generateSounds(): SoundSeed[] {
  ...
  return EXISTING_SOUNDS.map(({ id, title, artist, waveformPeaks }) => ({
    id,
    title,
    artist,
    waveformPeaks,
